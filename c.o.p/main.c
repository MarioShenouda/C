...
char* swap_line(char *first_string,char*secound_string);
...
int main()
{
 ...
 int inputnew=100,number=100,f=0,m=0,search=0,symbol=0,other=0,tab=0,v=0,w=0,a=0,find_lines=0,find_character=0,l=1,k,j;
  ...
  while(1)
{
  ...
  }   if(wahl=='s')
  {
   ...
   //  newstring[f-1]= swap_line(newstring[f-1],newstring[m-1]);
...
int charachter_sentence(char*text){
 ...
 char* swap_line(char *first_string,char*secound_string){
   if(first_string == NULL || secound_string == NULL)
    ...
    int first_array = 0, second_array = 0,length_first=0,length_secound=0;
    char temp;
    ...
    if(first_string[first_array] == '\0'){
        ...
        first_string = (char *)realloc(first_string,sizeof(char) * (length_first + length_secound));
        ...
        first_string[first_array++] = '\0';
        secound_string = (char *)realloc(secound_string,sizeof(char) * (length_secound+length_first));
    ...
    for(input=0;string[input]!='\0';input++)
       length++;
        ...
        char *new_string = (char *)realloc(string,sizeof(char) * (length + 5));
         // char *new_string = (char *)malloc(sizeof(char) * (length + 4));
